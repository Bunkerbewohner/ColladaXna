//--------------------------------------------------------------------------------
// Shader Generic-AcDcEcPhongScSp, generated by EffectCode at 12.03.2011 13:51:19
//--------------------------------------------------------------------------------
float4x4 World;
float4x4 WorldIT;
...
float3 DiffuseColor = float3(0.3098039,0.6470588,0.8235294);
...
struct VertexShaderOutput
{
	...
	float4 PositionWS : TEXCOORD0; // Position in World Space
	float3 Normal : NORMAL;
};
...
float4 PixelShaderFunction(PixelShaderInput pin) : COLOR
{
	...
	if (dt != 0)
		specular += DirLight1Color * pow(max(0,(2 * dot(L,N) * dot(N,E) - dot(E,L))), SpecularPower);
	...
	if (dt != 0)
		specular += DirLight2Color * pow(max(0,(2 * dot(L,N) * dot(N,E) - dot(E,L))), SpecularPower);
	...
	if (dt != 0)
		specular += DirLight3Color * pow(max(0,(2 * dot(L,N) * dot(N,E) - dot(E,L))), SpecularPower);
