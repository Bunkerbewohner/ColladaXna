//--------------------------------------------------------------------------------
// Shader Generic-AcEcNpOpPhongSmSpTx, generated by EffectCode at 11.03.2011 14:26:00
//--------------------------------------------------------------------------------
float4x4 World;
float4x4 WorldIT;
...
float4 PixelShaderFunction(PixelShaderInput pin) : COLOR
{
	...
	if (dt != 0)
		specular += DirLight1Color * tex2D(SpecularMapSampler, pin.TexCoord) * pow(max(0,(2 * dot(L,N) * dot(N,E) - dot(E,L))), SpecularPower);
	...
	if (dt != 0)
		specular += DirLight2Color * tex2D(SpecularMapSampler, pin.TexCoord) * pow(max(0,(2 * dot(L,N) * dot(N,E) - dot(E,L))), SpecularPower);
	...
	if (dt != 0)
		specular += DirLight3Color * tex2D(SpecularMapSampler, pin.TexCoord) * pow(max(0,(2 * dot(L,N) * dot(N,E) - dot(E,L))), SpecularPower);
