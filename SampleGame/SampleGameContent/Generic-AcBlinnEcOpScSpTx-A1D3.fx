//--------------------------------------------------------------------------------
// Shader Generic-AcBlinnEcOpScSpTx, generated by EffectCode at 11.03.2011 14:18:33
//--------------------------------------------------------------------------------
float4x4 World;
float4x4 WorldIT;
...
float4 PixelShaderFunction(PixelShaderInput pin) : COLOR
{
	...
	if (dt != 0)
		specular += DirLight1Color * pow(max(0,dot(normalize(E + L),N)), SpecularPower);
	...
	if (dt != 0)
		specular += DirLight2Color * pow(max(0,dot(normalize(E + L),N)), SpecularPower);
	...
	if (dt != 0)
		specular += DirLight3Color * pow(max(0,dot(normalize(E + L),N)), SpecularPower);
